...
async def _compute_route(start: str, end: str, tank_range: float, mpg: float,
    ...
    # Exact DP oracle (Gas Station Problem, Khuller et al. 2007).
    # Serve whichever plan is cheaper; report the optimality gap.
    optimizer_used = "greedy"
    optimality_gap = 0.0
    try:
        stops_exact = fuel_optimizer.optimize_fuel_stops_exact(
            stations_on_route, total_distance=total_miles,
            tank_range=tank_range, mpg=mpg,
        )
        if stops_exact is not None and stops:
            g_cost = sum(s["cost"] for s in stops)
            e_cost = sum(s["cost"] for s in stops_exact)
            optimality_gap = round(max(0.0, g_cost - e_cost), 2)
            if stops_exact and e_cost < g_cost - 0.005:
                stops = stops_exact
                optimizer_used = "exact_dp"
    except Exception as exc:
        logger.warning("Exact DP oracle failed — serving greedy plan: %s", exc)

    analytics = fuel_optimizer.get_route_analytics(
        stations_on_route, stops, total_miles, tank_range, mpg
    )
    analytics["optimizer_used"] = optimizer_used
    analytics["optimality_gap_usd"] = optimality_gap
...
def _render_map_html(result: dict) -> str:
  ...
  <div style="font-size:10px;font-weight:700;color:var(--mut);white-space:nowrap;text-align:right;min-width:42px">{dist_txt}<br><span style="font-size:9px;font-weight:400">mi {step['cum_mi']}</span></div>
    ...
    # Optimality row (exact DP oracle)
    oz_used = analytics.get("optimizer_used", "greedy")
    oz_gap  = analytics.get("optimality_gap_usd", 0.0)
    if oz_used == "exact_dp":
        optimality_row = f'<div style="display:flex;align-items:center;gap:5px;margin-bottom:10px;padding:6px 9px;background:rgba(168,85,247,.1);border:1px solid rgba(168,85,247,.25);border-radius:8px;font-size:11px;font-weight:600;color:#C4B5FD">&#9889; Exact DP plan served &mdash; ${oz_gap} cheaper than greedy heuristic</div>'
    else:
        optimality_row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:10px;padding:6px 9px;background:rgba(16,185,129,.07);border:1px solid rgba(16,185,129,.18);border-radius:8px;font-size:11px;font-weight:600;color:#6EE7B7">&#10003; Provably optimal &mdash; verified by exact DP oracle (Gas Station Problem), gap $0.00</div>'
  ...
  <div style="font-size:9px;font-weight:700;color:var(--mut);text-transform:uppercase;letter-spacing:.8px;padding:4px 6px 6px">Road Quality &mdash; {rql} ({rqi}/10)</div>
  <div style="background:var(--card);border:1px solid var(--brd);border-radius:9px;padding:8px 10px;margin-bottom:10px">
  ...
  {dead_html}
    ...
    terrain_sev_json = json.dumps(terrain.get("severity", "none"))
    terrain_regions_json = json.dumps(terrain.get("regions", []))
...
<body>
  ...
  <script>
    ...
    var terrainSev={terrain_sev_json};
    var terrainRegions={terrain_regions_json};
...
def _render_home_html() -> str:
    # Captured verbatim from the rendered page (raw string: backslashes literal)
