...
optimize_fuel_stops_exact(...)
    -> provably-optimal plan via the Gas Station Problem DP
       (Khuller, Malekian & Mestre, "To Fill or Not to Fill", 2007)
...
# ---------------------------------------------------------------------------
# Exact DP oracle — Gas Station Problem
# (Khuller, Malekian & Mestre, "To Fill or Not to Fill", 2007)
#
# Provably cost-optimal for the fixed-route model. Two structural lemmas
# bound the search space:
#   L1: if a CHEAPER station is within tank range, buy just enough to reach
#       the NEAREST such station (excess fuel should be bought there instead).
#   L2: if the next stop is PRICIER, fill the tank completely.
# State = (station index, arrival fuel). Arrival fuel takes O(n) distinct
# values, so memoization gives O(n^2) states. Used as an oracle alongside
# the greedy: the cheaper plan is served, and the gap is reported.
# ---------------------------------------------------------------------------

def optimize_fuel_stops_exact(
    stations_on_route: List[Dict],
    total_distance: float,
    tank_range: float = 500.0,
    mpg: float = 10.0,
    ...
    """
    Returns the provably-optimal stop plan (same shape as optimize_fuel_stops),
    [] for a direct trip, or None when no feasible plan exists.
    Decisions use effective (detour-adjusted) prices; billing uses retail.
    """
    capacity = tank_range / mpg
    for s in stations_on_route:
        s['_ep'] = _effective_price(s, capacity, mpg)

    nodes = sorted(_cluster_stations(stations_on_route),
                   key=lambda s: s['distance_from_start'])
    n = len(nodes)
    d = [s['distance_from_start'] for s in nodes]
    p = [s['_ep'] for s in nodes]

    EPS = 0.5                      # reachability slack, miles (same as greedy)
    INF = float('inf')
    f0 = capacity * max(0.0, min(1.0, current_fuel_fraction))

    if total_distance <= f0 * mpg + EPS:
        return []                  # direct trip — no purchase needed

    memo: Dict[Tuple[int, float], Tuple[float, Any]] = {}

    def solve(i: int, g: float) -> Tuple[float, Any]:
        """Min cost from node i with arrival fuel g; move = ('end'|k, buy)."""
        key = (i, round(g, 4))
        if key in memo:
            return memo[key]
        memo[key] = (INF, None)    # guard (graph is forward-only anyway)

        rem = total_distance - d[i]
        need_end = rem / mpg
        best_cost, best_move = INF, None

        # Option: buy just enough to finish (never overbuy for the last leg)
        if rem <= tank_range + EPS:
            buy_end = min(max(0.0, need_end - g), capacity - g)
            best_cost, best_move = buy_end * p[i], ('end', buy_end)

        reach_hi = d[i] + tank_range + EPS

        # L1 — nearest strictly cheaper station within range: forced move
        nearest_cheaper = -1
        ...
        if nearest_cheaper >= 0:
            k = nearest_cheaper
            hop = (d[k] - d[i]) / mpg
            buy = max(0.0, hop - g)
            arr = max(0.0, min(capacity, g + buy) - hop)
            sub, _ = solve(k, arr)
            cost = buy * p[i] + sub
            if cost < best_cost:
                best_cost, best_move = cost, (k, buy)
        else:
            # L2 — everything in range is pricier: fill the tank, branch on next stop
            buy = capacity - g
            for k in range(i + 1, n):
                if d[k] > reach_hi:
                    break
                hop = (d[k] - d[i]) / mpg
                arr = max(0.0, capacity - hop)
                sub, _ = solve(k, arr)
                cost = buy * p[i] + sub
                if cost < best_cost:
                    best_cost, best_move = cost, (k, buy)

        memo[key] = (best_cost, best_move)
        return memo[key]

    # Virtual start: no purchase possible — branch over stations within f0 range
    best_start, start_k = INF, -1
    for k in range(n):
        if d[k] > f0 * mpg + EPS:
            break
        arr = max(0.0, f0 - d[k] / mpg)
        sub, _ = solve(k, arr)
        if sub < best_start:
            best_start, start_k = sub, k
    if start_k < 0 or best_start == INF:
        return None                # dead zone — no feasible plan

    # Reconstruct (arithmetic mirrors solve() exactly so memo keys match)
    stops: List[Dict] = []
    i = start_k
    g = max(0.0, f0 - d[i] / mpg)
    for _ in range(n + 2):
        _, move = memo[(i, round(g, 4))]
        if move is None:
            return None
        target, buy = move
        if buy > 0.001:
            stops.append({
                'station': nodes[i],
                'gallons': round(buy, 2),
                'cost':    round(buy * nodes[i]['retail_price'], 2),
            })
        if target == 'end':
            return stops
        hop = (d[target] - d[i]) / mpg
        g = max(0.0, min(capacity, g + buy) - hop)
        i = target
    return None                    # safety: should be unreachable
